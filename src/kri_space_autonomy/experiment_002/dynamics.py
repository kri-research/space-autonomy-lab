...
def _continuous_events(
    ...
    for current_t in grid[1:]:
        ...
        if previous_v * current_v < 0.0:
            roots.append(_bisect(velocity, previous_t, current_t))
        elif current_v == 0.0:
            roots.append(current_t)
        previous_t = current_t
        previous_v = current_v
    critical = sorted({0.0, dt_s, *roots})
    ranges = [(time_s, range_at(time_s)) for time_s in critical]
    minimum_range = min(value for _, value in ranges)
    maximum_range = max(value for _, value in ranges)
    target = effectiveness * command if state.propellant > 0.0 else 0.0
    delta = state.achieved_acceleration_mps2 - target
    acceleration_candidates = [0.0, dt_s]
    ...
    if state.range_m <= config.collision_range_m:
        ...
        for (left_t, left_r), (right_t, right_r) in zip(ranges, ranges[1:], strict=False):
            if min(left_r, right_r) <= config.collision_range_m:
    ...
    if required <= state.propellant + 1e-15:
        ...
        if collision_local is not None:
            ...
            collision_state = TruthState(
                ...
                collision_state.relative_velocity_mps,
                collision_state.propellant,
                collision_state.achieved_acceleration_mps2,
            )
            return PropagationResult(
                collision_state,
                config.collision_range_m,
                max(state.range_m, collision_state.range_m),
                maximum_abs_velocity,
                collision_state.time_s,
                collision_state.propellant <= 0.0,
            )
        end = state_at(state, command, effectiveness, process_acceleration_mps2, dt_s, config)
        return PropagationResult(
            end, minimum, maximum, maximum_abs_velocity, None, end.propellant <= 0.0
        )
