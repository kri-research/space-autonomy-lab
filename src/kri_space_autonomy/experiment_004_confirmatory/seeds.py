...
SEED_DIRECTORY = Path("experiments/004-confirmatory/seeds")
RESULT_DIRECTORY = Path("results/experiment-004-confirmatory")
...
def _expected_contract(study: ConfirmatoryConfig) -> dict[str, Any]:
    return {
        ...
        "materialization_prerequisite": (
            ...
            "before the partition-44 generator is invoked."
        ),
...
def validate_seed_contract(
    ...
    *,
    ...
    if seed_directory.exists():
        errors.append("partition_44_seed_directory_exists")
    if result_directory.exists():
        errors.append("confirmatory_result_directory_exists")
    historical = _historical_root_ids(project_root)
    overlap = sorted(root_id for root_id in historical if root_id.startswith("experiment004:44:"))
    if overlap:
        errors.append("historical_partition_44_root_overlap")
    return {
        "passed": not errors,
        ...
        "historical_partition_44_overlap": len(overlap),
...
def materialize_confirmatory_scenario(
    ...
    *,
    ...
    """Derive one partition-44 scenario; never call this during design freeze."""
    ...
    root_id = f"experiment004:44:{case.case_code:03d}:{replicate:04d}"
    unsigned = {
        "schema_version": study.schema_version,
        "partition_code": partition,
        "case_id": case.id,
...
def validate_materialized_confirmatory_seeds(
    ...
    *,
    ...
    for row in rows:
        ...
        rederivation_errors += int(
            canonical_json(row) != canonical_json(expected.to_dict())
        )
    ...
    expected_replay = [
        f"experiment004:44:{case.case_code:03d}:{replicate:04d}"
        for case in cases
    ...
    if any(not root_id.startswith("experiment004:44:") for root_id in root_ids):
...
def assert_materialization_targets_absent(root: str | Path) -> None:
    ...
    if (project_root / SEED_DIRECTORY).exists() or (project_root / RESULT_DIRECTORY).exists():
        raise RuntimeError("refusing pre-existing partition-44 seed or result path")
...
def materialize_confirmatory_seeds(
    ...
    *,
    ...
    """Write the exact partition-44 schedule once after verified design freeze."""
    ...
    if not verification["passed"]:
        raise RuntimeError("refusing partition-44 materialization before verified freeze")
    if (
        ...
        raise RuntimeError("partition-44 materializer received a non-frozen identity")
    ...
    try:
            ...
            for case in cases:
                for replicate in range(study.roots_by_stratum[case.id]):
                    ...
                    if scenario.root_seed_id in observed or scenario.root_seed_id in historical:
                        raise RuntimeError("partition-44 root identity is not unique and disjoint")
        ...
        if len(observed) != study.planned_blocks:
            raise RuntimeError("partition-44 root count drift")
        replay_root_ids = [
            f"experiment004:44:{case.case_code:03d}:{replicate:04d}"
            for case in cases
            for replicate in range(study.replay_roots_per_stratum)
