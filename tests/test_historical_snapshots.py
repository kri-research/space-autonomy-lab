...
def test_real_catalogue_explains_eleven_and_retains_exact_gap():
    ...
    assert report["catalogue_complete"] is False
    assert report["experimental_provenance_verified"] is False
    assert verifier.exit_code(report) == 1
    gap = next(r for r in report["results"] if r["status"] == verifier.GAP)
    assert gap["expected_sha256"] == verifier.E002B_EXPECTED
    assert gap["current_sha256"] == verifier.E002B_OBSERVED
    assert gap["historical_commit"] is None
    assert gap["expected_bytes_available_in_git"] is False
    assert gap["external_record_matched"] is False
...
def test_cli_returns_one_for_known_gap_with_no_runtime_imports():
    ...
    assert report["status"] == "INCOMPLETE_PROVENANCE"
...
def test_matched_external_payload_does_not_waive_public_git_gap(monkeypatch):
    ...
    assert row["expected_bytes_available_in_git"] is False
