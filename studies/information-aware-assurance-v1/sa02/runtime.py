...
Adapted from pinned SA01 iaa/runtime.py; original source and results are unchanged.
The controller gets delivered packets only; the guard gets a measurement-conditioned outer set and
...
from iaa.plant import flow, observe
from iaa.types import Command, ExecutionRecord, StateBox, Status, Uncertainty, identity, primitive
...
def run(fixture, sensor_contract=None):
    ...
    baseline = Baseline()
    observer = Observer(prior, sensor_contract)
    history = []
    estimates = []
    update_timings = []
    ...
    for t in range(0, fixture.horizon_ms + 1, DT_MS):
        if t:
            if history and history[-1].action == action:
                history[-1] = replace(history[-1], end_ms=t)
            else:
                history.append(HistorySegment(t - DT_MS, t, action))
        ...
        if t % CYCLE_MS == SNAPSHOT_MS and t + 100 < fixture.horizon_ms:
            ...
            if spent + 5 > fixture.work_budget_mj:
                ...
                estimate = observer.update(tuple(packets), tuple(history), t)
                update_timings.append(perf_counter_ns() - measured_start)
                estimates.append(estimate)
                info = checker_input(estimate)
                ...
                detail = {
                    ...
                    "measurement_set_update": estimate.status,
                    "observation_set": primitive(estimate),
                    "observed_true_state_in_outer_set": estimate.contains_state(truth),
                    "evaluation_only_state_at_snapshot": [round(x, 12) for x in truth],
    ...
    summary = {
        ...
        "assumptions_valid": assumptions_valid,
        "assumptions_valid_scope": "dynamics_only_sensor_model_reported_separately",
        "guard_coverage_scope": (
            "conditional_on_dynamics_and_observation_contract_not_operational_safety"
        ),
    ...
    summary["observation_updates"] = len(estimates)
    summary["update_status_counts"] = {
        s: sum(e.status == s for e in estimates) for s in sorted({e.status for e in estimates})
    }
    summary["maximum_representation_cells"] = max((len(e.cells) for e in estimates), default=0)
    summary["observed_true_state_exclusions"] = sum(
        not r.details["observed_true_state_in_outer_set"]
        for r in records
        if r.event == "decision_snapshot"
    )
    summary["latent_hypotheses_at_last_update"] = (
        estimates[-1].diagnostics["hypotheses"] if estimates else []
    )
    summary["measured_host_timing_is_not_simulated_latency"] = True
    return summary, tuple(records), tuple(update_timings)
