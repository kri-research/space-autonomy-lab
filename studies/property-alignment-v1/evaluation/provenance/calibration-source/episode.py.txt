...
def mean_shortcut(info):
    ...
    return {
        ...
        "full_set_evidence": full,
        "false_safe_for_full_set": bool(
            result["status"] == "certified_common_prefix"
            and full is not None
            and any(h.get("status") == "validated_violation" for h in full.get("hypotheses", []))
        ),
        "full_set_unresolved": bool(
            result["status"] == "certified_common_prefix"
            and full is not None
            and full["status"] != "certified_common_prefix"
            and not any(
                h.get("status") == "validated_violation" for h in full.get("hypotheses", [])
            )
        ),
        "classification_version": "evidence-separated/2",
...
def run_case(payload, *, include_native_baselines=True, qualification_record=None):
    ...
    qual = qualification(info) if qualification_record is None else qualification_record
    ...
    result = {
        ...
        "timing_scope": "local processor policy timing; excludes qualification, certificate recheck and comparators; not WCET or target hardware",
        "certificate_independence": "Fixed-action replay and exact dual recheck share the tested candidate/interval implementation. No independent implementation or human review is asserted.",
